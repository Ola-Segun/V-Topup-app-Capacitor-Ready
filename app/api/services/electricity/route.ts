...
export async function POST(request: NextRequest) {
  try {
    ...
    const { provider, meterNumber, amount } = await request.json()

    if (!provider || !meterNumber || !amount || amount <= 0) {
    ...
    const { success: debitSuccess, error: debitError } = await transactionService.debitWallet(
      user.id,
      amount,
      `Electricity payment for meter ${meterNumber}`,
      reference
    )
    ...
    try {
      const paymentResult = await paymentService.processElectricityPayment({
        provider,
        meterNumber,
        amount,
        reference
      ...
      if (paymentResult.success) {
        ...
        return NextResponse.json({
          ...
          units: paymentResult.data?.units
        })
      } else {
